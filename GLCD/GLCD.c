...
void inv_PutChar( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor )
{
	uint16_t i, j;
    uint8_t buffer[16], tmp_char;
    GetASCIICode(buffer,ASCI);  /* 取字模数据 */
...
void inv_GUI_Text(uint16_t Xpos, uint16_t Ypos, uint8_t *str,uint16_t Color, uint16_t bkColor)
{		uint8_t TempChar;
		uint8_t *tmp = str;
		int count;
		
		for(count = 0;*tmp!= 0;count++,tmp++);
		
		for(count--;count>=0;count--)
    {
        TempChar = str[count];  
        inv_PutChar( Xpos, Ypos, TempChar, Color, bkColor );    
        if( Xpos < MAX_X - 8 )
        {
            Xpos += 8;
        } 
